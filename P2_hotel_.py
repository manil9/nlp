...
custom_title = "<h1 style='color: white; text-align: center;'> Analysis for Hotel Review</h1>"
...
def project_members_button():
    if st.sidebar.button('Project Members'):
        with st.expander("Meet the Team"):
            st.write("- <span style='color: blue;'>Gugloth Vijay</span>", unsafe_allow_html=True)
            st.write("- <span style='color: blue;'>Manil</span>", unsafe_allow_html=True)
            st.write("- <span style='color: blue;'>Pooja</span>", unsafe_allow_html=True)
            st.write("- <span style='color: blue;'>Saloni</span>", unsafe_allow_html=True)
...
if st.button("Predict"):
    ...
    # Apply custom CSS to change the text color for the predicted sentiment
    if predict == 'Positive':
        sentiment_text_color = "green"  # Change to the color you prefer for positive sentiment
    else:
        sentiment_text_color = "red"  # Change to the color you prefer for negative sentiment

    st.markdown(
        f'<p style="color: {sentiment_text_color}; font-weight: bold;">The Sentiment of the review is {predict} {Reaction[0]}</p>',
        unsafe_allow_html=True
    )
...
if st.sidebar.button("Visualize Word Cloud"):
...
if st.sidebar.button("Visualize N-gram"):
    st.sidebar.header("N-gram Chart Visualization")
